...
Defines the server-side integration gateway service that acts as a **secure gatekeeper** for all outbound operations → ERPNext financial payloads. Implements idempotency, retry with exponential backoff, JWT auth, and complete audit.
...
│   ├── idempotency.py       (MD5 key derivation + guard)
...
|--------|----------|------|-------------|
| `POST` | `/api/integration/sales-order` | internal | Enqueue Sales Order payload |
...
> **Internal** = Op-01/Op-02 services authenticate with service-level JWT tokens; endpoints are not internet-exposed.
...
- **Create:** Sales Order, Purchase Invoice, Sales Invoice, Cost Center
...
**Flow:** Before enqueue, the bus computes the idempotency key and checks `integration_outbox`. A matching key in `DONE` state → reject as duplicate (409). Prevents double-billing and overlapping accounting entries.
...
1. Provision `shipping_integration_bus` DB (schema in `database_plan.md`)
