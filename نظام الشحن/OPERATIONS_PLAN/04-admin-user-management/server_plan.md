...
Defines the FastAPI **Admin console service** that exposes the administrative interface to create branches, register employees, provision user accounts, assign roles, and mirror the RBAC model with ERPNext (which remains the permission authority).
...
│   ├── roles.py             (role browse + ERPNext pull)
│   ├── permissions.py       (permission matrix viewer)
│   └── acl_sync.py          (manual sync trigger + status)
├── services/
│   ├── rbac_service.py      (role resolution → JWT claims)
│   ├── erpnext_acl_service.py (ERPNext Employee/User/Role sync)
│   └── audit_service.py     (trail writer)
├── workers/
│   └── acl_replication.py   (Celery: pull roles, push accounts)
├── models/
└── config.py
```
...
| `POST` | `/api/admin/acl/sync` | **SYSTEM-ADMIN** | Trigger full push/pull with ERPNext |
| `GET` | `/api/admin/acl/sync/{id}` | admin | Sync attempt status + errors |
...
### 4.2 `erpnext_acl_service.py` — Accounting System Mirror

Handles the two-way ACL sync with ERPNext v15:
...
| Pull roles | `Role Profile` / `User Role` inventory | IN |
| Deactivate account | Disable `User` in ERPNext | OUT |
...
|--------|--------------------|----------|
| `acl_replication` | on-demand + daily | Pull ERPNext roles/permissions → `permission_matrix` |
| `account_provisioner` | event | Create ERPNext Employee/User/Role after local create |
...
| Role pull conflicts | Conflict resolution by `erpnext_role` unique key |
...
4. First-run `POST /api/admin/acl/sync` to pull base roles (`direction = IN`)
