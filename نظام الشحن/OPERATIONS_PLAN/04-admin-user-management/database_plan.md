...
Defines the administration database that powers the **Admin Console** — the interface for creating branches, registering employees, provisioning user accounts, and controlling role-based access control (RBAC). Roles and permissions are **defined and maintained in the accounting system (ERPNext)** and synced down to the platform; branch/employee/user objects are mirrored up to ERPNext as the source-of-truth financial entities.

---

## 2. Isolation & Security

- Dedicated admin schema within the operations PostgreSQL (with PostGIS)
- **ERPNext is the role/permission authority** — the platform enforces a synced mirror, never an independent divergent ACL
...
## 3. Role & Permission Model (ERPNext-Driven)

```
ERPNext (Authority)                        Platform (Enforcement)
Entreprise Roles + Permissions          platform_roles (mirror)
        │  sync ▼                                │
        └─────────────► user_branch_roles ───────► JWT claims
                       permission_matrix  ───────► route guards
```

| Design Rule | Description |
|-------------|-------------|
| **Single source of truth** | New roles/permissions are created in ERPNext, then propagated to `platform_roles` + `permission_matrix` |
...
### 4.4 `platform_roles` — ERPNext-Synced Role Mirror
...
### 4.7 `acl_sync_log` — ERPNext Authority Sync Trail
...
Supplied by ERPNext and pulled in on first sync (`direction = IN`):

| platform_role | erpnext_role | Typical Grants |
|---------------|--------------|----------------|
| `SYSTEM-ADMIN` | Administrator | all resources, all branches |
...
```
ERPNext (role authority)                 Admin Console
   platform_roles   ──PULL──▶      platform_roles + permission_matrix
   (permissions)                        │
                                        ▼
   leverages JWT claims on every
   platform API (all OP domains)
                                        ▲
Branch create ──OUT─▶ ERPNext Cost Center + Warehouse
Employee create ──OUT─▶ ERPNext Employee + User
Role assignment ──OUT─▶ ERPNext User Role add
                                        │
