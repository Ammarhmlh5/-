...
def provision_supplier(supplier_id: str, claims=Depends(get_current_access_claims),
    ...
    config = get_config()
    erp_id = None
    if config.APP_ENV != "testing" and config.MARKET_ERP_KEY:
        from ..services.erpnext_client import ErpNextError, queue_supplier
        try:
            queued = queue_supplier(supplier.to_dict())
        except ErpNextError as error:
            tenant_service.fail_provisioning(db, job, "ERPNEXT_PROVISION_FAILED")
            raise HTTPException(status_code=502, detail=str(error))
        write_audit(db, request_id=new_request_id(), operation="SUPPLIER_PROVISION_QUEUED",
                    result="SUCCESS", tenant_id=supplier.supplier_id, actor_id=claims["sub"])
        return {
            "message": "Supplier provisioning queued",
            "status": "PENDING",
