...
product_category_table = Table(
    "product_categories",
    Base.metadata,
    Column("product_id", UUID(as_uuid=False), ForeignKey("marketplace_catalog.product_id", ondelete="CASCADE"), primary_key=True),
    Column("category_id", UUID(as_uuid=False), ForeignKey("catalog_categories.category_id", ondelete="CASCADE"), primary_key=True),
)
...
class Product(Base):
    ...
    categories = relationship("CatalogCategory", secondary=product_category_table, back_populates="products")
...
class CatalogCategory(Base):
    """تصنيف كتالوج مملوك لمورد محدد، وليس تصنيفاً عاماً مشتركاً."""

    __tablename__ = "catalog_categories"

    category_id = Column(UUID(as_uuid=False), primary_key=True, default=_uuid)
    supplier_id = Column(UUID(as_uuid=False), ForeignKey("suppliers.supplier_id", ondelete="CASCADE"), nullable=False)
