# OP-01: Server Plan — Supplier Catalog & Sourcing Operations
...
Defines the backend service for supplier catalog, stock and sourcing operations. It does not expose a direct database connection to customer applications and does not own customer orders or payments.

---

## 2. Service Architecture

A separately deployable FastAPI service (`catalog_service`) or a module in the initial modular monolith. It is separated from logistics, customer orders and ERPNext at the data-access boundary.
...
| `POST` | `/api/sourcing-requests` | customer-service | Request an unlisted product |
| `POST` | `/api/sourcing-requests/{id}/quotes` | staff/vendor | Add a supplier quote |
| `POST` | `/api/sourcing-requests/{id}/approve` | customer-service | Approve a quote and publish a price snapshot |
...
### 4.3 `sourcing_service.py`

- Quote lifecycle: `DRAFT → SUBMITTED → APPROVED → EXPIRED/CANCELLED`
- On approval, emits a sourcing event to the customer/order service. OP-03 receives only the resulting financial command.
...
|--------|----------|----------|
| `catalog_projection` | On catalog publication | Publish approved catalog data to the customer read model |
...
1. Provision catalog PostgreSQL instance (`shipping_catalog`) and object storage
2. Apply DB schema (`database_plan.md`)
3. Deploy FastAPI instance on `:8000`
4. Start Celery workers + Redis broker
5. Point the supplier portal at catalog endpoints and the customer backend at the read model/API
