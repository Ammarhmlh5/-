...
class IntegrationOutbox(db.Model):
    """أوامر مالية تنتظر الترحيل غير المتزامن إلى ERPNext."""
    ...
    erp_document_name = db.Column(db.String(120))
    last_error = db.Column(db.Text)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)


class IntegrationAudit(db.Model):
    """سجل محاولات إرسال أوامر التكامل، append-only منطقياً."""
    __tablename__ = 'integration_audit'

    id = db.Column(PostgreSQLUUID(as_uuid=False), primary_key=True, default=lambda: str(uuid.uuid4()))
    outbox_id = db.Column(PostgreSQLUUID(as_uuid=False), db.ForeignKey('integration_outbox.id'), nullable=False)
    action = db.Column(db.String(40), nullable=False)
    attempt = db.Column(db.Integer)
    http_status = db.Column(db.Integer)
    response_summary = db.Column(db.Text)
    actor_reference = db.Column(db.String(120))
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    outbox = db.relationship('IntegrationOutbox', backref=db.backref('audit_entries', lazy='dynamic'))